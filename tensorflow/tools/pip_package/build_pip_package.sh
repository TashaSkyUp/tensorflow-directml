...
function copy_dml_redist_files() {
  ...
  if is_windows; then
    ...
    # The source filename will be DirectML.dll (stable redist) or DirectML.<SHA>.dll (preview redist).
    ...
    echo "DML Version = '$dml_version'"
    # The source filename will be libdirectml.so (stable redist) or libdirectml.<SHA>.so (preview redist).
